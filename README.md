> ⚠️ **Breaking Change**: Версии выше 1.0.0 (например, 2.0.0+) содержат критические изменения и **несовместимы** с версией 1.0.0 и ниже. Все генерируемые сервисы теперь используют ESM модули вместо CommonJS. См. раздел [Breaking Changes](#-breaking-changes-в-версиях-выше-100) для подробностей.
...
1. **Базовые Файлы Сервиса**:
    - `service.ts`: Основной файл сервиса с импортом JSON схемы через ESM.
    - `start.ts`: Точка входа для запуска сервиса.
    - `index.ts`: Основной индексный файл с экспортом клиента сервиса.
